        ...
        <div className="container mt-5">
          <div className="row g-4">
            {sortData.slice(0,visible).map((sortitem,index)=> <div className="col-md-3" key={index}>
              <Link to={'/gameDetails/'+sortitem.id} className='text-decoration-none text-muted'>
              <div className="sortitem  pb-3 shadow-lg bg-gradient mb-5 ">
             <img src={sortitem.thumbnail} className='w-100 mb-3' alt="" />
               <div className='position-relative px-2'>
               <h4 className='px-2'>{sortitem.title.slice(0,20)}</h4>
                <span className="badge text-bg-success px-3 mx-3 py-2 position-absolute end-0 bottom-0">Free</span>
               </div>
               <div className='d-flex px-2 justify-content-around '>
               <i className="fas fa-plus-square px-2 text-muted"></i>
               <span className=' rounded p-1 me-2 text-muted text-bg-dark'>{sortitem.genre}</span> {sortitem.platform=="PC (Windows)"?<i className="fa-brands fa-windows text-muted"></i>:<i className="text-muted fa-solid fa-window-maximize"></i>}
               </div>
              </div>
              </Link>
            </div> )}
